...
#
#
pd.concat([pd.DataFrame(y_train.reset_index(drop=True)),pd.DataFrame(y_pred_train)], axis=1, ignore_index=True).to_csv('result_DNN_trainset.csv', index=False)
pd.concat([pd.DataFrame(y_test.reset_index(drop=True)),pd.DataFrame(y_pred_valid)], axis=1, ignore_index=True).to_csv('result_DNN_testset.csv', index=False)