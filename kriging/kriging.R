...
sh193.grid <- expand.grid(LON = seq(from = LON.range[1], to = LON.range[2], by = 2000),
                                LAT = seq(from = LAT.range[1], to = LAT.range[2], by = 2000))
...
Kriging_result<- autoKrige(formula = PM25~1,input_data = sh193, new_data = sh193.grid )
plot(Kriging_result)
...
myKrige <- data.frame(Kriging_result$krige_output@coords, 
